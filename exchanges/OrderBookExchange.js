...
module.exports = class OrderBookExchange {
  ...
  async computePrice(symbol, desiredAmount, isSell, fee) {
    ...
    if (book === '_createCanonicalOrderBook not implemented') {
      ...
      const { avgPrice, totalPrice } = isSell
        ? calculatePricing(book.bids, desiredAmount)
        : calculatePricing(book.asks, desiredAmount)
     ...
     * @param {*} desiredAmount the lot size to be filled
     */
    function calculatePricing(orders, desiredAmount) {
      let avgPrice = 0
      let totalPrice
      for (let i = 0; i < orders.length; i += 1) {
        ...
        if (desiredAmount > orders.lotAmount) {
          continue
        }
