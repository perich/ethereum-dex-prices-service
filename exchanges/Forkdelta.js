...
module.exports = class Forkdelta extends OrderBookExchange {
  constructor() {
    ...
    this.tickerUrl = `${FORKDELTA_URL}/returnTicker`
    this.name = 'Forkdelta'
  }
...
function Service() {
  ...
  self.updateOrders = (newOrders, token, user) => {
    ...
    ********************** */
    ...
    self.state.orders = {
      sells: self.state.orders.sells.sort((a, b) => a.price - b.price || a.amountGet - b.amountGet),
      buys: self.state.orders.buys.sort((a, b) => b.price - a.price || b.amountGet - a.amountGet),
    }
    self.state.myOrders = {
      sells: self.state.myOrders.sells.sort((a, b) => a.price - b.price || a.amountGet - b.amountGet),
      buys: self.state.myOrders.buys.sort((a, b) => b.price - a.price || b.amountGet - a.amountGet),
    }
  }
  ...
  self.updateTrades = (newTrades, token, user) => {
    ...
    newTradesTransformed.forEach(x => {
      if (!self.state.trades.find(y => y.txHash === x.txHash)) {
        ...
        if (x.buyer.toLowerCase() === user.addr.toLowerCase() || x.seller.toLowerCase() === user.addr.toLowerCase()) {
          self.state.myTrades.push(x)
        }
      }
    })
    self.state.trades = self.state.trades.sort((a, b) => new Date(b.date) - new Date(a.date) || b.amount - a.amount)
    self.state.myTrades = self.state.myTrades.sort((a, b) => new Date(b.date) - new Date(a.date) || b.amount - a.amount)
  }
  ...
  self.getEtherDeltaBalance = (token, user) =>
    new Promise((resolve, reject) => {
      self.contractEtherDelta.balanceOf(
        token === 'ETH' ? '0x0000000000000000000000000000000000000000' : token.addr,
        user.addr,
        (err, result) => {
          if (err) reject(err)
          resolve(result)
  ...
  self.takeOrder = (user, order, fraction) =>
    new Promise((resolve, reject) => {
      self
        .getNextNonce(user)
        .then(nonce => {
  ...
  self.createOrder = (side, expires, price, amount, token, user) => {
    ...
    const sign = (msgToSignIn, privateKeyIn) => {
      ...
      const privateKey =
        privateKeyIn.substring(0, 2) === '0x' ? privateKeyIn.substring(2, privateKeyIn.length) : privateKeyIn
    ...
    const amountGet = side === 'buy' ? self.toWei(amountBigNum, token.decimals) : self.toWei(amountBaseBigNum, 18)
    const amountGive = side === 'sell' ? self.toWei(amountBigNum, token.decimals) : self.toWei(amountBaseBigNum, 18)
    const orderNonce = Number(
      Math.random()
        .toString()
        .slice(2),
    )

    const unpacked = [contractAddr, tokenGet, amountGet, tokenGive, amountGive, expires, orderNonce]

    const condensed = pack(unpacked, [160, 160, 256, 160, 256, 256, 256])
  ...
  self.toEth = (wei, decimals) => new BigNumber(String(wei)).div(new BigNumber(10 ** decimals))
  self.toWei = (eth, decimals) => new BigNumber(String(eth)).times(new BigNumber(10 ** decimals)).floor()
}
